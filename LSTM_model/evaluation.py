...
import torch
import numpy as np
from data_processing import x_input, eth_price, y_price_change
from model import model, criterion, last_c0, last_h0
...
def evaluation_MSEloss():
  ...
  for day in range(len(eval_pred)):
    pred = eval_pred[day]
    newpred = eth_price[day] * pred + eth_price[day] # using prev day's price to calculate pred price
    pred_price.append(newpred)
  print(pred_price)

# evaluate based on monetary loss in all-in strategy 
def evaluation_ALLIN(eval_pred, pred_price):
...
def evaluation_PROPORTIONAL(eval_pred, pred_price):
  USDC_balance = 200
  ETH_balance = 0.5
