...
def data_processing():
    scaler = MinMaxScaler()
    # read eth price
    inFile = 'all_data\y_ethprice csv.csv'
    y_price_change = []
    eth_price = []
    ...
    # read cex flow
    inFile = 'all_data\MWUA_cexflow.csv'
    x_cexflow = []
    with open(inFile) as f:
        for row in f:
            if row.split(',')[1]:
    ...
    # read eth balance
    inFile = 'all_data\MWUA_balance.csv'
    x_balance = []
    with open(inFile) as f:
        for row in f:
            if row.split(',')[0]:
    ...
    inFile = 'all_data\MWUA_eth2stable.csv'
    x_eth2stables = []
    with open(inFile) as f:
        for row in f:
            if row.split(',')[0]:
    ...
    inFile = 'all_data\x_ethdexvolume csv.csv'
    x_dexvolume = []
    with open(inFile) as f:
        for row in f:
            if row.split(',')[2]:
    ...
    inFile = 'all_data\x_aavestableir csv.csv'
    x_interest_rate = []
    with open(inFile) as f:
        for row in f:
            ir = row.split(',')[1]
