...
def MWUA() -> list[str]:
    ...
    for wallet in eth_balance.keys():
        ...
        balance = 0
        for value in eth_balance[wallet_lower].values():
            balance += value
        avg_balance[wallet_lower] = balance / 365
    ...
    for week_start in daterange:
        ...
        for wallet in selected_addresses:
            ...
            if week_cexflow != 0:
                cexflow_prediction_degree = np.abs(week_cexflow) / avg_balance[wallet_lower] * np.abs(price_change) * _lambda # prediction discounted by purchase power
            ...
            start_balance = eth_balance[wallet_lower][week_start.strftime("%Y/%m/%d")]
            end_balance = eth_balance[wallet_lower][week_end.strftime("%Y/%m/%d")]
            if start_balance != 0:
            ...
            start_eth2stable = eth2stable_ratio[wallet_lower][week_start.strftime("%Y/%m/%d")]
            end_eth2stable = eth2stable_ratio[wallet_lower][week_end.strftime("%Y/%m/%d")]
        ...
        plt.scatter(np.arange(len(weights_list)), weights_list)
