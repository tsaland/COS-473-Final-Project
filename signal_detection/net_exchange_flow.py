...
def get_cexflow(exchange_wallets, selected_addresses, w3):
    ...
    end_block = convert_date_to_blockNumber(end_date+timedelta(days=1), w3) # starts from the first block in 2022
...
# Convert net_cexflow to a csv
def net_cexflow_to_csv(net_cexflow):
   with open('net_cexflow.csv', 'w') as f:
        for date_str, transfer_out, transfer_in in net_cexflow.items():
            f.write(date_str + ',' + str(transfer_out) + ',' + str(transfer_in) + '\n')
